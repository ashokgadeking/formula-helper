...
def _log_card_rects(filtered: list) -> list:
    """Return list of (card_rect, real_index) for displayed entries (newest first)."""
    rects = []
    shown = list(reversed(filtered))[:LOG_MAX_SHOW]
    y = LOG_CARD_Y0
...
def render_log(surf: pygame.Surface, mix_log: list,
               ...
               log_view_date: str, trends_btn: Button = None) -> None:
    surf.fill(C_BG)
...
# ── Settings gear icon ─────────────────────────────────────────────────────
GEAR_RECT = pygame.Rect(10, 10, 36, 36)


def draw_gear(surf: pygame.Surface) -> None:
    """Draw a small gear icon in the top-left corner."""
    cx, cy = GEAR_RECT.center
    r_outer, r_inner = 14, 8
    color = C_DIM
    # Outer circle
    pygame.draw.circle(surf, color, (cx, cy), r_outer, 2)
    # Inner circle
    pygame.draw.circle(surf, C_BG, (cx, cy), r_inner)
    pygame.draw.circle(surf, color, (cx, cy), r_inner, 2)
    # Teeth (8 small rects around the circle)
    import math
    for i in range(8):
        angle = i * math.pi / 4
        tx = cx + int((r_outer - 1) * math.cos(angle))
        ty = cy + int((r_outer - 1) * math.sin(angle))
        pygame.draw.rect(surf, color, pygame.Rect(tx - 3, ty - 3, 6, 6))


# ── Settings screen ───────────────────────────────────────────────────────
def render_settings(surf: pygame.Surface, countdown_secs: int, ss_timeout_min: int,
                    timer_plus: Button, timer_minus: Button,
                    ss_plus: Button, ss_minus: Button,
                    back_btn: Button, reset_btn: Button,
                    close_btn: Button) -> None:
    surf.fill(C_BG)

    draw_text(surf, "SETTINGS", 28, C_WHITE, (W // 2, 30), bold=True)

    # ── Countdown timer setting ────────────────────────────────────────────
    y = 100
    draw_text(surf, "Expiry Timer", 18, C_DIM, (W // 2, y - 10))
    if countdown_secs >= 60:
        timer_label = f"{countdown_secs // 60} min"
    else:
        timer_label = f"{countdown_secs} sec"
    draw_text(surf, timer_label, 40, C_BLUE,
              (W // 2, y + 30), bold=True)
    timer_plus.draw(surf, font_size=30)
    timer_minus.draw(surf, font_size=30)

    # ── Screensaver timeout setting ────────────────────────────────────────
    y2 = 240
    draw_text(surf, "Screensaver Timeout", 18, C_DIM, (W // 2, y2 - 10))
    draw_text(surf, f"{ss_timeout_min} min", 40, C_YELLOW,
              (W // 2, y2 + 30), bold=True)
    ss_plus.draw(surf, font_size=30)
...
def _aggregate_trends(mix_log: list, range_name: str) -> tuple:
    """Return (daily_dict, total_ml, count, num_days) for the given range."""
    from datetime import timedelta
...
def render_trends(surf: pygame.Surface, mix_log: list,
    ...
    daily, total_ml, count = _aggregate_trends(mix_log, range_name)
...
def main(fullscreen: bool, simulate: bool) -> None:
    ...
    # ── Main screen buttons (3×2 grid, 100px tall each, pinned to bottom) ──
    main_btn_h = 100
    col_w = W // 3
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if mode == AppMode.MAIN:
                # Gear icon tap
                if event.type == pygame.MOUSEBUTTONUP and GEAR_RECT.collidepoint(event.pos):
                    mode = AppMode.SETTINGS
                    btn_changed = True
                elif main_start60_btn.handle_event(event):
            ...
            elif mode == AppMode.SETTINGS:
                if set_timer_plus.handle_event(event):
                    if countdown_secs < 60:
                        countdown_secs = min(countdown_secs + 10, 60)
                    else:
                    ...
                    if countdown_secs <= 60:
                        countdown_secs = max(countdown_secs - 10, 10)
                    else:
                    ...
                    mode = AppMode.MAIN
                    btn_changed = True
                elif set_reset_btn.handle_event(event):
                    countdown_end = 0.0
                    mixed_at_str = ""
                    mixed_ml = 0
                    ntfy_sent = False
        ...
        if mode == AppMode.MAIN:
            ...
            frame_key = (mode, countdown_secs, ss_timeout_min,
                         set_timer_plus.pressed, set_timer_minus.pressed,
                         set_ss_plus.pressed, set_ss_minus.pressed,
                         set_back_btn.pressed, set_reset_btn.pressed,
                         set_close_btn.pressed)
        elif mode == AppMode.TRENDS:
        ...
        if frame_key != last_frame_key or btn_changed:
            if mode == AppMode.MAIN:
                ...
                render_settings(surf, countdown_secs, ss_timeout_min,
                                set_timer_plus, set_timer_minus,
                                set_ss_plus, set_ss_minus,
                                set_back_btn, set_reset_btn, set_close_btn)
            elif mode == AppMode.TRENDS:
